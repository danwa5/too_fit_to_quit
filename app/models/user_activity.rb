class UserActivity < ActiveRecord::Base
  ...
  scope :monthly_breakdown, ->(year=Date.today.year) {
    find_by_sql("
      ...
      left join (
        select extract(month from start_time) as month, count(id) as total_#{year-3}, round(avg(duration/60)) as avg_duration_#{year-3}
        from user_activities
        where activity_type = 'Activity::FitbitRun' and date_part('year', start_time) = #{year-3}
        group by month
      ) ua3 using (month)
      left join (
